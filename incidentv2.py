...
def ppa(mysql, conn):
    query = 'Select patient.patient_id, account.account_id, person.person_id from patient left join account on account.account_id = patient.account_id left join person on account.primary_person_id = person.person_id where patient.patient_id > 1'
...
def getIncidentid(mysql, conn, personId, patientId, accountId, incidentTypeid, incidentnameId):
    ...
    mysqlWorker(mysql, conn, queries[1].format(patientId, incidentnameId, incidentTypeid), 2)
...
def incidentname(mysql, conn):
    ...
    queries = ['Select incidentname_id from incidentname where incidentname_id > 1', 'Insert into incidentname (a_description) values ("{}")']
    data = mysqlWorker(mysql, conn, queries[0], 1)
    if len(data) < 5:
        for i in range(5):
...
def incident_type_ins(mysql, conn):
    queries = ['Select incident_type_id from incident_type where incident_type_id > 1', "Insert into incident_type (a_description) values ('{}')", 'Select incident_type_id from incident_type where incident_type_id > 1']
    data = mysqlWorker(mysql, conn, queries[0], 1)
    while len(data) < 4:
        desc = nameGen()
