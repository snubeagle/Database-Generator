...
def personIdf(mysql, conn):
    query = 'select person_id from person where person_id not in (select person_id from patient) and person_id not in (select person_id from person_ins_tie)'
    people = []
    data = mysqlWorker(mysql, conn, query, 1)
    for i in data:
        people.append(i[0])
    personId = random.choice(people)
    return personId

def pitIns(mysql, conn, startDate, personId):
    insuranceId = insuranceIdf(mysql, conn)
    personId = personIdf(mysql, conn)
...
def run_ppit():
    ...
    mysql = conn.cursor()
    x = int(raw_input("How many patient/person/insurance links would you like? "))
    for a in range(int(x)):
        startDate = "{}-{:02}-{:02}".format(2015, random.randint(1, 4), random.randint(1, 28))
        personId = personIdf(mysql, conn)
        pitId = pitIns(mysql, conn, startDate, personId)
        ppitIns(mysql, conn, pitId, startDate, personId)
